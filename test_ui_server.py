...
class ConnectedMode(unittest.TestCase):
    ...
    def test_06_public_application_works_without_signing_in(self):
        ...
        page.fill('#apply-form input[name="name"]', "Connected Applicant / Demo")
        page.fill('#apply-form input[name="email"]', "connected@example.test")
        page.fill('#apply-form input[name="audience"]', "9000")
        page.fill('#apply-form input[name="engagement"]', "2.5")
        page.fill('#apply-form input[name="fit"]', "80")
        page.fill('#apply-form input[name="niche"]', "Connected testing")
        page.fill('#apply-form input[name="url"]', "https://example.com/connected")
        page.locator('#apply-form input[name="consent"]').check()
        page.locator('#apply-form button[type="submit"]').click()
        page.locator("#login-form").wait_for()
        self.assertIn("connected@example.test", [c["email"] for c in self.server_state()["creators"]])
        page.close()
